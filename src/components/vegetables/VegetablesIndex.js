...
class VegetablesIndex extends React.Component {
  ...
  componentDidMount() {
    ...
    const searchFromRedirect = this.props.location.state
      ? this.props.location.state.detail
      : "";
    axios
      .get("/api/vegetables")
      .then(res =>
        this.setState({ vegetables: res.data, searchTerm: searchFromRedirect })
      )
  ...
  onChange({ target: { name, value, dataset, innerHTML } }) {
    ...
    value
      ...
      : this.setState({ [dataset.name]: innerHTML });
  }
  ...
  filterVegetables() {
    ...
    const filteredArr = this.state.vegetables.filter(veg => {
      ...
      return (
        (!veg.pickUpAppointment ||
          (veg.pickUpAppointment &&
            veg.pickUpAppointment.appointmentStatus !== "completed")) &&
        re.test(veg.title) &&
        (type.test(veg.typeOfVeg) || typeSearch === "All")
      );
    });
    return filteredArr;
  }
  ...
  displayVegetables(filteredArr) {
    ...
    return vegCards.length !== 0 ? (
      vegCards
    ) : (
      <h2>
        Your search returned no Results. Maybe you could grow this veg for
        us?...
      </h2>
    );
  }
  ...
  submitSearch(e) {
    ...
    axios
      .get("/api/vegetables")
      .then(res => {
        const filteredArr = res.data.filter(veg =>
          new RegExp(this.state.searchTerm, "i").test(veg.title)
        );
